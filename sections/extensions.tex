...
\item (Even faster block relaying for fast confirmations) To archive even faster block relaying several techniques can be combined, including the two described coded by in Luke-Jr in \url{https://gitorious.org/bitcoin/luke-jr-bitcoin/commit/4e54ea804ccdd2223e622497f0d46cceb27b9d22} and \url{https://gitorious.org/bitcoin/luke-jr-bitcoin/commit/0fc6cf6d35e1710d4b3f2ffadb9fb9b39176b167}. The alt-coin NimbleCoin \url{http://NimbleCoin.com} combines several of such techniques described in ~\cite{FastCoin5} to achieve a 5 second block interval with low stale block rate. The implemented improvements are:   
\begin{itemize}
\item Transactions are delayed 5 seconds on each miner’s block transaction queue to allow the fastest possible block verification, because transactions are already present in the pools of every node of the network.
\item New network command to spread block headers with time critical priority (similar to Luke-Jr proposal)
\item New network command to spread block transactions hash list immediately after block header propagates.
\item Mining over block headers with unverified transactions with a 5 seconds fall-back.
\item New message transport layer with message-slicing allowing two parallel sessions with distinct priorities. This allows block headers to be sent over the high priority session and interrupt whatever message was being transmitted over the low priority session.
\item Local optimal block routing based on peer priorities. Nodes prioritize their peers based on a score that tracks how many times a new block was first advertised by each peer. Each time a new block header arrives it's sent to the peers in order or their priority. This effectively embeds a dynamic DAG in the random network graph, where miners are connected with path of short latencies. 
\end{itemize}
\item (low-trust dynamic centralization for almost instant confirmations) 
Generally peer-to-peer systems provide greater resilience against attacks and accidental disruptions than centralized systems. Nevertheless decentralization comes with a price: consensus must be reached by voting or by proof-of-work, and both solutions tend to be slower than traditional centralized decisions. Instant payments requires instant consensus, and that is something that has been difficult to achieve in pure peer-to-peer networks. Currently one of the main factors that prevents reducing the block interval to less than 5 seconds is the packet delay in block broadcast. This is influenced by the length of the route and the interaction between the TCP reliability and congestion control protocols, application level manipulation of the data, propagation delay, serialization, data protocols, routing and switching, queuing, buffing, bandwidth and noise. The MinCen protocol ~\cite{MinCen} tries to overcome this problem. The MinCen network behaves like centralized network electing a single master node, but the network can disperse into fully distributed one if something goes wrong with the master node. Nevertheless MinCen protocol does not attempt to centralize mining, the MinCen network chooses a master node that only decides which is the next block of the best chain when there are competing chains. Everyone then follows this decision. But if the master node tries to cheat or loose network connectivity, then every miner starts looking for another master node, in such a way that individual decisions converge to a new global selection. During the transition period, miners become their own master nodes, so the network never performs worse than a Bitcoin-like network.

\item (The Private Automatic Miner Backbone Protocol (PAMBA))
The need for direct connection between the main miner pools or solo-miners was discussed several times in the forums.  A miner backbone provides not only benefits to the network as a whole, but benefits for those miners that establish such connections. A backbone decreases the chances of blocks being orphaned, so miners also have a strong incentive to have direct connections between them and announce a solved block as soon as possible. Also the network benefits from the backbone because a backbone decreases the probability of a block-chain split between two competing subnets because of an accidental or deliberate loss of connectivity. There are several possible attacks to the Bitcoin network widely analyzed that try to isolate nodes, such as the Sybil attack or the Timejacking attack. A direct backbone would allow miners to either go around the attacked nodes or detect if the other miners are unreachable and therefore they are being isolated. A backbone can be created manually: two mining pool administrators meet in an IRC channel, they convince each other that they are who they say they are, they convince each other what their hashing power is, they exchange PGP public keys, they verify the public keys in a public keystore,  and finally they exchange encrypted and signed e-mails containing their IP addresses. This has many drawbacks: it's difficult to keep miners anonymous, does not scale well and it's static. The PAMBA and PAMBA+ protocols ~\cite{PAMBA} provide a way for miners to create an automatic backbone without disclosing their network hashing power or IP addresses to unwanted parties.

\item (Increasing transactions per second)
One of the limiting factors of Bitcoin scalability is the need for verifying digital the signatures included in each transaction. Normally each node must check every digital signature present in each block. There was some work on nodes that check only a subset of the transactions and an alert system for cheating attempts [cite], but this is orthogonal to the core optimization of the digital signature verification process. Some digital signature schemes allow batch verification, but generally they require that all signatures in the batch belong to the same public key, which is not the case of a normal Bitcoin block. The MAVE ~\cite{MAVE} digital signature system and the MAVEPAY ~\cite{MAVEPAY} payment protocol allow to verify digital signatures without performing no more than three hash digests of small messages per payment, relying on using the block-chain as a secure time-stamped commitment channel. As a comparison, using the same CPU and bandwidth resources, MAVEPAY achieves 700 tps where Bitcoin achieves 10 tps.  
